...
class WaveFieldDataset(data.Dataset):

    def __init__(self, opt):
        super(WaveFieldDataset, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        pre_gt_path = self.paths[index-1]['gt_path']
        pre_img_bytes = self.file_client.get(pre_gt_path, 'gt')
        pre_img_gt = imfrombytes(pre_img_bytes, float32=True)
        # BGR to RGB, HWC to CHW, numpy to tensor
        pre_img_gt = img2tensor(pre_img_gt, bgr2rgb=False, float32=True)
        ...
        # flow
        if self.flow_dir:
            flow_path = self.paths[index-1]['flow_path']
            ...
            if flow_path is not None:
                ...
                flow_conf_path = self.paths[index-1]['flow_conf_path']
                ...
                # flow[flow_conf] = 5   # 这个能把置信度直接可视化出来，记得后面加上，先不加看看能不能跑出来哦

            else:
                flow = torch.tensor(-1e5)

        # print(img_gt.shape)  # torch.Size([2073600, 3])
        # print(self.grid.shape)  # torch.Size([2073600, 2])
        # print(flow.shape)  # torch.Size([2073600, 2])
        # exit()

        warp_grid = self.grid + flow
        warp_grid = warp_grid * 2 - 1
        warp_grid = warp_grid[:, [1,0]]
        warp_grid = warp_grid.view(1080, 1920, 2).unsqueeze(0)

        warp_img_gt = F.grid_sample(pre_img_gt.unsqueeze(0), warp_grid, mode='bilinear', padding_mode='border', align_corners=True)
        # 注意此时这个流是上一帧的，与codef不同
        warp_img_gt = warp_img_gt.squeeze().permute(1,2,0).view(1080*1920, 3)



        # print(img_gt.shape)
        # cv2.imwrite('0.png', warp_img_gt.view(1080, 1920, 3).detach().cpu().numpy()*255)
        # cv2.imwrite('1.png', img_gt.view(1080, 1920, 3).detach().cpu().numpy()*255)
        # exit()

        if self.opt['phase'] == 'train':
            return {'gt': torch.stack([img_gt, warp_img_gt], dim=0), 'gt_path': gt_path, 'grid': self.grid, 't_i': t_i, 'flow': flow, 'num_frame': torch.tensor(len(self.paths))}
        else:
            return {'gt': img_gt, 'gt_path': gt_path, 'grid': self.grid, 't_i': t_i, 'flow': flow, 'num_frame': torch.tensor(len(self.paths))}
