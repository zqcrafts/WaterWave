...
def flow_visualize(flow, grid_r_l1, mk_fw_l1,step):
    grid_r_l1 = mk_fw_l1 * grid_r_l1
    grid_r_l1 = -grid_r_l1.view(1, 1080, 1920, 2).permute(0,3,1,2).detach().cpu().numpy()
    grid_r_l1 = grid_r_l1[:,[1,0]]
    grid_r_l1[:,0,:,:] = grid_r_l1[:,0,:,:] * 1920
    grid_r_l1[:,1,:,:] = grid_r_l1[:,1,:,:] * 1080

    flow = mk_fw_l1 * flow
    flow = -flow.view(1, 1080, 1920, 2).permute(0,3,1,2).detach().cpu().numpy()
    flow = flow[:,[1,0]]
    flow[:,0,:,:] = flow[:,0,:,:] * 1920
    flow[:,1,:,:] = flow[:,1,:,:] * 1080  
    flow = flow + grid_r_l1
    # 假设flow是你的光流张量，形状为(1, 2, H, W)
    H, W = flow.shape[2], flow.shape[3]
    flow_uv = flow[0].transpose(1, 2, 0)
    flow_u, flow_v = flow_uv[..., 0], flow_uv[..., 1]
    # 计算Middlebury颜色编码
    flow_color = 255-compute_color(flow_u, flow_v)
    cv2.imwrite(f'/home/ubuntu/data/code/Temporal_consistency/results/recify_flow/{step}.png', flow_color)


def flow_warp_visualize(flow, grid_r_l1, mk_fw_l1,step):
    grid_r_l1 = mk_fw_l1 * grid_r_l1
    grid_r_l1 = -grid_r_l1.view(1, 1080, 1920, 2).permute(0,3,1,2).detach().cpu().numpy()
    grid_r_l1 = grid_r_l1[:,[1,0]]
    grid_r_l1[:,0,:,:] = grid_r_l1[:,0,:,:] * 1920
    ...
    flow_uv = flow[0].transpose(1, 2, 0)
    # flow_u, flow_v = flow_uv[..., 0], flow_uv[..., 1]
    np.save(f'/home/ubuntu/data/code/Temporal_consistency/results/recify_flow/{step:06d}.npy', flow)
    ...
    # flow[0, :, :] /= w
    # flow[1, :, :] /= h
    # mesh_y, mesh_x = torch.meshgrid(torch.linspace(0, 1, h), torch.linspace(0, 1, w))
    # grid = torch.FloatTensor(torch.stack((mesh_x, mesh_y), dim=0).unsqueeze(0))
    
    # grid = grid * 2 -1
    # grid = torch.clamp(torch.tensor(grid), -1, 1)
    # warp_grid = grid + flow
    # warp_frame = F.grid_sample(images[i].unsqueeze(0), warp_grid.permute(0, 2, 3, 1), mode='bicubic', padding_mode='border', align_corners=False)
    # warp_frame = warp_frame.squeeze(0).permute(1, 2, 0)
    # cv2.imwrite(f'/home/ubuntu/data/code/Temporal_consistency/results/warp_recify_flow/{self.step}.png', warp_frame.byte().numpy())
...
def compute_color(u, v):
    ...
    # 将角度从弧度转换为0-180的度数，用于HSV颜色空间的色调通道
    hsv_ang = ang * (180 / np.pi / 2)
    
    # 将大小归一化到0-255，用于HSV颜色空间的值通道
    hsv_val = cv2.normalize(mag, None, 0, 255, cv2.NORM_MINMAX)
    
    # 创建HSV格式的颜色表示，色调表示角度，饱和度设置为255，值表示大小
...
class WaveField_conv_test(nn.Module):
    def __init__(self, annealed_step=None, annealed_begin_step=None):
        super(WaveField_conv_test, self).__init__()
    ...
    def forward(self, grid, t_i, flow, num_frame):
        ...
        mask[mask!=0] = 1
        H_t = mask * H_t * 5
        # flow_warp_visualize(flow, grid_r_l1, mk_fw_l1, self.step)
