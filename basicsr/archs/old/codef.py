...
class ImplicitVideoSystem(LightningModule):
    ...
    def deform_pts(self, ts_w, grid, encode_w, step=0, i=0):
        if hparams.deform_hash:
            ts_w_norm = ts_w / self.seq_len  # 对时序做了个归一化
            ts_w_norm = ts_w_norm.repeat(grid.shape[0], 1)
            # print(ts_w_norm.shape)  # torch.Size([2073600, 1])
            # print(self.embeddings.keys())  # dict_keys(['xyz', 'xyz_w', 'w_0']) 测试的时候没有退火key
    ...
    def forward(self,
        ...
        flow_loss_list = []
        deform_list = []
        for i in range(self.num_models):  # 每帧mask的个数
            # print(ts_w.shape)  # torch.Size([1, 1])
            # print(grid.shape)  # torch.Size([518400, 2])
            # print(encode_w)  # True
            # exit()
            ...
            if not self.training and self.hparams.canonical_dir is not None:
                ...
                # print(deformed_grid.shape)  # torch.Size([518400, 2])
                # print(grid_new.shape)  # torch.Size([518400, 2])
                # exit()
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(self.num_models):
            ...
            # print(self.hparams.bg_loss)  # 0.003
            # print(self.hparams.mask_dir)  # None
            ...
            if ret.flow_loss[0] != 0:
                ...
                print(mk_flow_t.shape)  # torch.Size([16384])
                print(mk_flow_t)  # tensor([False, False, False,  ..., False, False, False], device='cuda:0')
                exit()
    ...
    def test_step(self, batch, batch_idx):
        ...
        # print(batch.keys())  # dict_keys(['rgbs', 'canonical_img', 'ts_w', 'grid', 'canonical_wh', 'img_wh', 'masks', 'flows', 'grid_c', 'reference', 'seq_len'])
        # print(batch_idx)  # 0,1,2,3...
        ts_w = batch['ts_w']
        grid = batch['grid']
        mk = batch['masks']
        grid_c = batch['grid_c']
        ...
        ret_n = self(ts_w, grid, self.hparams.encode_w, self.global_step)  # self指类的实例对象，forward, ret_n指result
        # print(ret_n.shape)  #  self._proc.stdin.write(vid.tostring()) ret是一个类，后面有ret_n.rgbs[0]  ret_n.deform[0]
        # print(ret_n.rgbs[0].shape)  # torch.Size([2073600, 3])
        # print(ret_n.deform[0].shape)  # torch.Size([2073600, 2])
