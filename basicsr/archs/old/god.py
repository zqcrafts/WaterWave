...
class CodeF_aneal_flow(nn.Module):
    def __init__(self, annealed_step=None, annealed_begin_step=None):
        super(CodeF_aneal_flow, self).__init__()
    ...
    def forward(self, grid, t_i, flow):
        ...
        # if is_train:
            ...
            # mk_flow_t = self.mk_t.unsqueeze(1)
            torch.set_printoptions(profile="full")
            # print(flow * mk_flow_t)
            # print(grid)
            # print(mk_flow_t.shape)
            flow_loss = (deformed_grid.clone().unsqueeze(0) * mk_flow_t, deformed_grid_new.unsqueeze(0) * mk_flow_t)  # detach关键
            # flow_loss = (deformed_grid.detach().unsqueeze(0), deformed_grid_new.detach().unsqueeze(0))
