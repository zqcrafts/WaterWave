...
class ImplicitVideo_Hash(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.spatial_encoder = tcnn.Encoding(n_input_dims=2,
                                     encoding_config=config["encoding_low_fre"])
        # self.temporal_encoder = tcnn.Encoding(n_input_dims=1,
        #                              encoding_config=config["encoding_low_fre"])
        
        self.decoder = tcnn.Network(n_input_dims=self.spatial_encoder.n_output_dims + 2,
                                    n_output_dims=3,
                                    network_config=config["network_deform"])

    def forward(self, x):
        input = x
        input = self.spatial_encoder(input)
...
class Deform_Hash3d(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = tcnn.Network(n_input_dims=self.spatial_encoder.n_output_dims + self.temporal_encoder.n_output_dims + 3,
                                    n_output_dims=2,
                                    network_config=config["network_deform"])

    def forward(self, x, step=0, aneal_func=None):

        input_spatial = self.spatial_encoder(x[:, 0:2])
        input_temporal = self.temporal_encoder(x[:, 2].unsqueeze(-1))

        input_spatial = torch.cat([x, input_spatial, input_temporal], dim=-1)
        x = self.decoder(input_spatial)
...
class DoubleFrePos(nn.Module):
    ...
    def forward(self, grid, t_i, flow):
        ...
        if is_train and flow.max() > -1e2:
            ...
            flow_loss = (deformed_grid.clone().unsqueeze(0) * mk_flow_t, deformed_grid_new.unsqueeze(0) * mk_flow_t) 

        # deformed_grid = (deformed_grid + 0.3) / 1.6
        output = self.VideoINR(deformed_grid)
