...
class Deform_Hash3d(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.encoder = tcnn.Encoding(n_input_dims=2,
                                     encoding_config=config["encoding_spatial"])
        # self.temporal_encoder = tcnn.Encoding(n_input_dims=1,
        #                              encoding_config=config["encoding_temporal"])
    ...
    def forward(self, x, step=0, aneal_func=None):
        ...
        input_spatial = self.encoder(x[:, 0:2])
        # input_temporal = self.temporal_encoder(x[:, 2])
        input_spatial = torch.cat([x, input_spatial], dim=-1)
