...
class WaveField_old(BaseModel):

    def __init__(self, opt):
        super(WaveField_old, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        self.num_frame = data['num_frame'].to(self.device)
    
    def optimize_parameters(self, current_iter):

        self.net_g.requires_grad_(True)
        self.optimizer_g.zero_grad()
        self.output, self.flow_mask = self.net_g(self.grid, self.t_i, self.flow, self.num_frame)
        
        # compute loss
        l_total = 0
        loss_dict = OrderedDict()
        # self.output = self.output.view(-1, 2, 1080, 1920, 3).permute(0,1,4,2,3)
        # self.gt = self.gt.view(-1, 2, 1080, 1920, 3).permute(0,1,4,2,3)
        ...
        if self.cri_pix:
            ...
            # print(self.gt.shape)
            l_pix = self.cri_pix(self.output, self.gt, self.flow_mask)
            l_total += l_pix
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.output, _ = self.net_g_ema(self.grid, self.t_i, self.flow)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.output, _ = self.net_g(self.grid, self.t_i, self.flow, self.num_frame)
            self.net_g.train()
