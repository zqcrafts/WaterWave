...
class WaveField(BaseModel):
    ...
    def __init__(self, opt):
        super(WaveField, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output = self.net_g(self.grid, self.t_i, self.flow, self.num_frame)
        ...
        if self.cri_flow: 
            l_flow = self.cri_flow(self.output, self.gt)
            l_total += l_flow
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.output = self.net_g_ema(self.grid, self.t_i, self.flow, self.num_frame)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.output = self.net_g(self.grid, self.t_i, self.flow, self.num_frame)
            ...
            # flops, params = profile(self.net_g, inputs=(self.grid, self.t_i, self.flow, self.num_frame))
